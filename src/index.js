...
client.once('clientReady', async () => {
  // If APPLICATION_ID is missing/invalid, derive from the logged-in application's ID
  const appIdFromClient = client.application?.id;
  if ((!APP_ID || !/^\d{17,}$/.test(APP_ID)) && appIdFromClient) {
    APP_ID = appIdFromClient;
    console.log(`[config] APPLICATION_ID was corrected to ${APP_ID} from client.application.id`);
  }
